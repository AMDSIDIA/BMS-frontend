  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        logWarn(`Impossible de parser le body d'erreur: ${parseError.message}`, 'API');
      ...
      if (response.status === 401) {
        logger.authError('Token invalide', new Error(errorData.error || 'Token expiré ou invalide'), errorData);
      ...
      const errorMessage = errorData.error || errorData.message || `Erreur HTTP ${response.status}: ${response.statusText}`;
    ...
    try {
      ...
      logError(`Erreur lors du parsing de la réponse: ${parseError.message}`, parseError, 'API');
    ...
    // Log détaillé de l'erreur
    logger.apiError(endpoint, error, undefined, {
      url: url,
    ...
    let enhancedMessage = `Erreur lors de la récupération des données: ${error.message}`;
    ...
    logError(`Message d'erreur amélioré: ${enhancedMessage}`, error, 'API');
